...
def download_style(style, start=1):
    ...
    a_pool.map(download_artist, tmp)
...
def download_artist(artist, start=1, print_info=False):
    ...
    for i, row in enumerate(tqdm(rows)):
        ...
        def extract_text(label, soup=soup):
            try:
                return ' '.join(
                    soup.find(text=f'{label.title()}:').parent.parent.text.split(':')[1].strip().split('\n'))
            except:
        ...
        try:
            ...
            if not os.path.isdir(f'./{artist}'):
                os.mkdir(f'./{artist}')
            urllib.request.urlretrieve(download_link+'!PinterestSmall.jpg', savepath)
        except IndexError:
        ...
        if print_info:
            ...
            print(
                f'author:\t{artist}\ntitle:\t{title}\ndate:\t{date}\nstyle:\t{style}\ngenre:\t{genre}\nmedia:\t{media}\ndimns:\t{dimensions}\ntag:\t{tag}\nloc:\t{location}')
        # saving to csv
        info = {'author': artist, 'title': title, 'date': date, 'style': style, 'genre': genre, 'media': media,
                'dimensions': dimensions, 'tag': tag, 'location': location, 'link': download_link,
                'image': f'{name_code}.jpg'}
